...
async function counting(delay = 500) {
  ...
  const step = true;
  let countingPsuedo = ['c ← 0', 'for i ← 0, a.length − 1 do', 'if (< condition >) then', 'c ← c + 1',' end i', 'end for', 'return c' ]
...
async function maximum(delay = 500) {
  setCountValue.replaceChildren();
  header.replaceChildren();
  let minimumPsuedo =['min ← a[0]', 'for i ← 0, a.length − 1 do', 'if (a[i] > max) then', 'max ← a[i]', 'end if', 'end for', 'return max' ];
  setCountValue.innerHTML +=`
  ...
  for (var i = 1; i < blocks.length; i += 1) {
    ...
    console.log('if the minimum is greater than the current value set the minimum to the current value')
  }
  enable();
...
async function minimum(delay = 500) {
  ...
  let minimumPsuedo =['min ← a[0]', 'for i ← 0, a.length − 1 do', 'if (a[i] < min) then', 'min ← a[i]', 'end if', 'end for', 'return min' ];
  setCountValue.innerHTML +=`
  <li id='setCountValue1'  class = 'listCss'>${minimumPsuedo[0]}</li>
  <li id='setCountValue2'  class = 'listCss'>${minimumPsuedo[1]}</li>
  <li id='setCountValue3'  class = 'listCss'>${minimumPsuedo[2]}</li>
...
async function summing(delay = 500) { 
  ...
  let summingPseudo =  ['c ← 0', 'for i ← 0, a.length − 1 do', 'if (< condition >) then', 's ← s + a[i]',' end i', 'end for', 'return s' ]
  setCountValue.innerHTML +=`
...
async function searchAlgorithm(delay = 500) {
  ...
  let pseudo = ['for i ← 0, a.length − 1 do', 'if (a[i] == e) then', 'return true', 'end if', 'end for', 'return false'];
  ...
  for (var i = 0; i < blocks.length; i += 1) {
    ...
    if(step2)
    {
      ...
      // document.getElementById("test2").style.backgroundColor = "#d8b6ff";
